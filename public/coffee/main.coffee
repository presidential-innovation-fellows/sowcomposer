$ ->
  ...
  $(".datepicker input").each ->
    if $(this).val() == ''
      $(this).closest(".controls, li").find(".tbd-checkbox").attr('checked', true)
...
$(document).on "click", "input.tbd-checkbox", ->
  input = $(this).closest(".controls, li").find(".datepicker input")
  if this.checked
    input.val('')
  else
    input.val(input.parent().data('date'))
...
$(document).on "click", "#add-custom-choice-btn", (e) ->
  ...
  newChoice.find(".choice-name input").val(text)
